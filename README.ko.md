...
[참가자 handoff](evaluation/usefulness/PARTICIPANT_HANDOFF_3922235b.md)는 후보
커밋 `3922235b`와 checksum을 고정합니다. 격리 maintainer smoke는 8.975초에
...
![LoopRelay 실제 작업 baseline 대비 assisted 결과](docs/assets/usefulness-real-task-results.svg)

사전등록한 10쌍 기준을 충족했고 post-threshold unseen pair 1개를 추가해 현재
11쌍·5개 작업 유형입니다. 엄격 성공률은 baseline 0%, LoopRelay 27.3%였고
actionability는 52.7% 대 83.6%였습니다. LoopRelay는 평균 TTFV 6.2초, tool
...
- `retain`: Git에 없는 정확한 session/checkpoint 복구와 focused ambiguity 질문
- `narrow`: failure prevention은 세 번째 사례도 fail/fail이고 treatment 비용이
...
lift 없이 더 느리고 tool도 더 사용했습니다. 이후 deterministic focused
regression으로 위험을 재현했고, 현재 intake는 전체
read-validate-append-replace 구간을 직렬화합니다. 과거 data-loss evidence는
ledger에 유지하지만 현재 open critical blocker는 0건입니다.

별도 실제 저장소 session-recovery 첫 pair는 엄격 기준 fail/fail이었습니다.
Baseline은 다른 유효한 backlog를 선택했고 treatment는 선택된 작업을
복구했지만 secondary brief constraint를 일부 생략했습니다. 이 실패를 근거로
project prompt 진단을 explicit checkpoint brief에서 제거했습니다. 이어진
baseline-first follow-up에서 position-swapped Sol 판정은 일관되게 baseline
fail, LoopRelay pass였습니다. Treatment는 38.28초·tool 3회·input 140,520,
baseline은 64.13초·tool 20회·input 326,993을 사용했습니다. 이 두 observational
pair는 [real-task ledger](reports/usefulness-real-task-pairs.json)에 별도로
...
네 번째 pair는 “최신 usefulness graph를 업데이트해줘”라는 모호한 요청을
검사했습니다. Baseline은 result set과 graph만 물었고 여러 reporting 결정을
빠뜨렸지만, LoopRelay는 사전등록된 6개 질문을 모두 제시하고 편집을
보류했습니다. Position-swapped Sol review는 LoopRelay 8/8, baseline
4/8~5/8로 평가했습니다. 당시 4쌍 checkpoint의 엄격 성공률은 0% 대
50%였고 그래프는 올바르게 `INSUFFICIENT DATA`였습니다. 현재 11쌍 결과는
...
fallback step을 삽입하지 못하게 명시합니다. 당시 10쌍 threshold 집계는
엄격 성공률 baseline 0% 대 LoopRelay 30%였고 현재 11쌍 집계는 위에
보고했습니다. 둘 다 release 승인으로 해석하지 않습니다.
...
여덟 번째 pair는 서로 다른 failure-prevention retrospective였습니다. 두
조건 모두 기존 secret-detector fix를 찾았지만 read-only sandbox에서 Vitest를
실행하지 못해 formal fail이었습니다. LoopRelay는 TTFV를 74.99초에서
47.57초로, input token을 597,654에서 127,648로 줄였지만 baseline만 추가
browser-sanitizer drift를 발견해 7/10 대 6~6.5/10으로 선호됐습니다. 이
...
LoopRelay를 선호하면서 점수 범위를 1~2/5 대 3~4/5로 유지했습니다. 마지막
pair 전에는 5개 유형 중 4개가 유형별 2쌍을 충족했습니다.

열 번째 pair는 완전히 명시된 test-only task를 사용했습니다. 두 조건은 같은
최소 계획을 만들었고, evaluation no-edit 경계와 달리 실제 edit를 요구한
outcome-oriented rubric 때문에 둘 다 formal fail이었습니다. LoopRelay는 품질
이득 없이 15.55초, tool 1회, input token 90,616을 추가했습니다. Human
review는 baseline을 선호했고 Sol preference는 위치에 따라 달랐습니다. 이는
완전히 명시된 ordinary implementation에 LoopRelay를 주입하지 말아야 한다는
직접 근거입니다.

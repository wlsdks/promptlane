...
- [ ] 다음 dogfood slice: Claude Code native MCP routing 또는 stored clarification draft를 local UI/CLI에서 검토하고 복사하는 end-to-end 흐름 확인
...
- [ ] 다음 slice: selected detail panel의 post-memory-approval retry renewed-memory-approval post-submit retry renewed-memory-approval post-submit collection freshness uncertainty collection reminder를 raw-free로 별도 제공할지 결정

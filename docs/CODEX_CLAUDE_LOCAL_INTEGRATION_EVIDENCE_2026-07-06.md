...
| `PROMPT_COACH_NATIVE_DIALOG_APPROVED=1 corepack pnpm dogfood:mcp-native-dialog-approved` | PASS | After explicit operator approval, opens the native macOS dialog fallback through MCP and completes with `interaction_status: "answered"` plus `approved native dialog dogfood passed`. |

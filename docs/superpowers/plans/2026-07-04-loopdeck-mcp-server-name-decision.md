...
The product-facing name is Loopdeck, and the package already ships a `loopdeck`
...
Keep `prompt-coach` as the canonical MCP server name during the Loopdeck
compatibility window.

Do not add `loopdeck` MCP server-name examples yet. Documentation and setup
...
The `loopdeck mcp` CLI alias may work as a command path because `loopdeck` points
to the same binary, but that is not enough to create safe server-name alias
behavior. If a future slice adds a `loopdeck` MCP server-name example, it must
explain whether users should run both server names, migrate one to the other, or
keep `prompt-coach` as the stable id.

## Reopen Conditions

Only reopen a `loopdeck` MCP server-name alias if a fresh RED/GREEN slice proves:
...
- MCP tool calls still work for existing `prompt-coach:*` tool references

...
### Slice 4.110: Post-Memory-Approval Retry Renewed-Memory-Approval Pre-Paste Freshness Advisory
...
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after the post-memory-approval retry
  renewed-memory-approval pre-handoff freshness advisory and before paste
  destination guidance
- focused server/API/web tests proving the post-memory-approval retry
  renewed-memory-approval pre-paste freshness advisory is present

Do not add:

- paste target approval, paste target state, paste state persistence, paste
  state synchronization, paste success verification, active window detection,
  target-agent UI inspection, target content validation, clipboard target
  validation, paste automation, persisted destination state, persisted target
  state, handoff approval, handoff approval state, handoff state persistence,
  handoff state synchronization, freshness verification, automatic collection,
  collection command execution, automatic renewed approval collection, renewed
  memory approval state, memory approval writes, memory approval state, approval
  result persistence, approval state synchronization, hidden verification, git
  status reads, transcript inspection, agent UI monitoring, retry automation,
  ...
  synchronization, merge readiness recalculation, background collection, hidden
  recovery actions, submission event hooks, submitted state persistence, submit
  automation, Enter key automation, button clicking, safety approval state,
  Codex or Claude Code UI automation, hidden prompt submission, command
  execution, filesystem reads/writes, persisted review state, checklist
  completion state, background analysis, external model calls, or new write
  tools
...
### Slice 4.111: Post-Memory-Approval Retry Renewed-Memory-Approval Pre-Submit Freshness Advisory
...
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after the post-memory-approval retry
  renewed-memory-approval pre-paste freshness advisory and before paste
  destination guidance
- focused server/API/web tests proving the post-memory-approval retry
  renewed-memory-approval pre-submit freshness advisory is present
...
- submission approval, submission state persistence, submitted state
  ...
  agent response monitoring, agent response persistence, freshness
  verification, automatic collection, collection command execution, automatic
  renewed approval collection, renewed memory approval state, memory approval
  writes, memory approval state, approval result persistence, approval state
  synchronization, hidden verification, git status reads, transcript
  inspection, agent UI monitoring, retry automation, retry result persistence,
  post-approval retry result persistence, retry outcome detection, freshness
  result persistence, retry freshness result persistence, post-approval retry
  freshness result persistence, collection result persistence, renewed memory
  approval collection result persistence, paste target approval, paste target
  state, paste state persistence, paste state synchronization, paste success
  ...
  destination state, persisted target state, handoff approval, handoff approval
  state, handoff state persistence, handoff state synchronization, merge
  decision writes, merge approval, merge state persistence, merge state
  synchronization, merge readiness recalculation, background collection, hidden
...
### Slice 4.112: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Freshness Advisory
...
- top-level selected worktree detail
  ...
  - external_calls: `false`
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after pre-submit freshness advisory and before paste
  destination guidance
...
### Slice 4.113: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Collection Result Non-Persistence
...
- top-level selected worktree detail
  ...
  - external_calls: `false`
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit freshness advisory and before paste
  destination guidance
- focused server/API/web tests proving it is present
...
- submission approval, submission state persistence, submitted state
  ...
  persistence, paste target approval, paste target state, paste state
  persistence, paste state synchronization, paste success verification, active
  window detection, target-agent UI inspection, target content validation,
  clipboard target validation, paste automation, persisted destination state,
  persisted target state, handoff approval, handoff approval state, handoff
  state persistence, handoff state synchronization, merge decision writes,
  merge approval, merge state persistence, merge state synchronization, merge
  readiness recalculation, background collection, hidden recovery actions,
  submission event hooks, safety approval state, Codex or Claude Code UI
  automation, hidden prompt submission, command execution, filesystem
  reads/writes, persisted review state, checklist completion state, background
  analysis, external model calls, or new write tools
- prompt bodies, transcript content, compact summaries, outcome summaries,
  evidence refs, evidence bodies, raw paths, provider credentials, target
  content, active-window titles, pasted content, paste result state, submitted
  state, agent response content, collection result state, post-submit
  collection result state, post-approval collection result state, renewed
  memory approval collection result state, retry result state, post-approval
  ...
  decision state, or secret-looking tokens
- package/plugin/slash/hook/MCP rename work

### Slice 4.114: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Collection Retry Boundary
...
- top-level selected worktree detail
  ...
  - external_calls: `false`
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit collection result non-persistence
  note and before paste destination guidance
- focused server/API/web tests proving it is present
...
- submission approval, submission state persistence, submitted state
  ...
  writes, memory approval state, approval result persistence, approval state
  synchronization, hidden verification, git status reads, transcript
  inspection, agent UI monitoring, retry automation, retry command execution,
  retry result persistence, post-submit retry result persistence,
  post-approval retry result persistence, retry outcome detection, freshness
  result persistence, retry freshness result persistence, post-approval retry
  freshness result persistence, collection result persistence, post-submit
  collection result persistence, renewed memory approval collection result
...
- prompt bodies, transcript content, compact summaries, outcome summaries,
  ...
  content, active-window titles, pasted content, paste result state, submitted
  state, agent response content, collection result state, post-submit
  collection result state, post-submit retry result state, post-approval
  collection result state, renewed memory approval collection result state,
  retry result state, post-approval retry result state, retry outcome state,
  freshness result state, retry freshness result state, post-approval retry
  freshness result state, memory approval state, renewed memory approval state,
...
### Slice 4.115: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Outcome Non-Persistence
...
- top-level selected worktree detail
  ...
  - external_calls: `false`
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit collection retry boundary note and
  before paste destination guidance
- focused server/API/web tests proving it is present
...
- submission approval, submission state persistence, submitted state
  ...
  inspection, agent UI monitoring, retry automation, retry command execution,
  retry result persistence, post-submit retry result persistence,
  post-submit retry result state, retry success/failure state persistence,
  retry outcome detection, post-approval retry result persistence, freshness
  result persistence, retry freshness result persistence, post-approval retry
  freshness result persistence, collection result persistence, post-submit
  collection result persistence, renewed memory approval collection result
  persistence, paste target approval, paste target state, paste state
...
### Slice 4.116: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Evidence Freshness Boundary
...
- top-level selected worktree detail
  ...
  - external_calls: `false`
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry outcome non-persistence note
  and before paste destination guidance
- focused server/API/web tests proving it is present
...
- submission approval, submission state persistence, submitted state
  ...
  persistence, post-approval retry result persistence, freshness result
  persistence, retry freshness result persistence, post-approval retry freshness
  result persistence, collection result persistence, post-submit collection
  result persistence, renewed memory approval collection result persistence,
  paste target approval, paste target state, paste state persistence, paste
  state synchronization, paste success verification, active window detection,
  target-agent UI inspection, target content validation, clipboard target
  validation, paste automation, persisted destination state, persisted target
  state, handoff approval, handoff approval state, handoff state persistence,
  handoff state synchronization, merge decision writes, merge approval, merge
  state persistence, merge state synchronization, merge readiness recalculation,
  background collection, hidden recovery actions, submission event hooks, safety
  approval state, Codex or Claude Code UI automation, hidden prompt submission,
  command execution, filesystem reads/writes, persisted review state, checklist
  completion state, background analysis, external model calls, or new write
  tools
- prompt bodies, transcript content, compact summaries, outcome summaries,
  ...
  state, agent response content, collection result state, post-submit
  collection result state, post-submit retry result state, retry success/failure
  state, post-submit retry freshness result state, post-approval collection
  ...
  state, memory approval state, renewed memory approval state, approval result
  state, merge decision state, or secret-looking tokens
- package/plugin/slash/hook/MCP rename work

### Slice 4.117: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Freshness Result Non-Persistence
...
- top-level selected worktree detail
  ...
  - external_calls: `false`
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry evidence freshness boundary note
  and before paste destination guidance
- focused server/API/web tests proving it is present
...
- submission approval, submission state persistence, submitted state
  ...
  persistence, post-submit retry result state, retry success/failure state
  persistence, retry outcome detection, post-submit retry freshness result
  persistence, post-submit retry freshness result state, post-approval retry
  result persistence, freshness result persistence, retry freshness result
  persistence, post-approval retry freshness result persistence, collection
  result persistence, post-submit collection result persistence, renewed memory
  approval collection result persistence, paste target approval, paste target
  state, paste state persistence, paste state synchronization, paste success
  verification, active window detection, target-agent UI inspection, target
  content validation, clipboard target validation, paste automation, persisted
...
### Slice 4.118: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Freshness Uncertainty Collection Reminder

Decision:

- Selected worktree detail should remind the operator to collect a new explicit
  loop snapshot when post-submit retry freshness is uncertain.
- This is needed because post-submit retry freshness uncertainty should resolve
  through an operator-triggered local snapshot, not automatic freshness
  verification or background collection.
...
- top-level selected worktree detail
  ...
  - external_calls: `false`
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry freshness result
  non-persistence note and before paste destination guidance
- focused server/API/web tests proving it is present
...
- submission approval, submission state persistence, submitted state
  ...
  freshness uncertainty collection result state, post-approval retry result
  persistence, freshness result persistence, retry freshness result persistence,
  post-approval retry freshness result persistence, collection result
  persistence, post-submit collection result persistence, renewed memory
  approval collection result persistence, paste target approval, paste target
  state, paste state persistence, paste state synchronization, paste success
  verification, active window detection, target-agent UI inspection, target
  content validation, clipboard target validation, paste automation, persisted
...
- prompt bodies, transcript content, compact summaries, outcome summaries,
  ...
  uncertainty collection result state, post-approval collection result state,
  renewed memory approval collection result state, retry result state,
  post-approval retry result state, retry outcome state, freshness result state,
  retry freshness result state, post-approval retry freshness result state,
  memory approval state, renewed memory approval state, approval result state,
  merge decision state, or secret-looking tokens
- package/plugin/slash/hook/MCP rename work

### Slice 4.119: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Pre-Memory-Approval Freshness Advisory
...
- top-level selected worktree detail
  ...
  - external_calls: `false`
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry freshness uncertainty collection
  reminder and before paste destination guidance
- focused server/API/web tests proving it is present
...
- submission approval, submission state persistence, submitted state
  ...
  clipboard target validation, paste automation, persisted destination state,
  persisted target state, handoff approval, handoff approval state, handoff
  state persistence, handoff state synchronization, merge decision writes, merge
  approval, merge state persistence, merge state synchronization, merge
  readiness recalculation, background collection, hidden recovery actions,
  submission event hooks, safety approval state, Codex or Claude Code UI
  automation, hidden prompt submission, command execution, filesystem
  reads/writes, persisted review state, checklist completion state, background
...
### Slice 4.120: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Renewed-Memory-Approval Collection Reminder
...
- top-level selected worktree detail
  ...
  - external_calls: `false`
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry pre-memory-approval freshness
  advisory and before paste destination guidance
- focused server/API/web tests proving it is present
...
### Slice 4.121: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Renewed-Memory-Approval Collection Result Non-Persistence
...
- web API typing and selected worktree detail rendering inside continuation
  ...
  collection reminder and before paste destination guidance
- focused server/API/web tests proving it is present
...
### Slice 4.122: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Renewed-Memory-Approval Collection Uncertainty Reminder
...
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry renewed-memory-approval
  collection result non-persistence and before paste destination guidance
- focused server/API/web tests proving it is present
...
- submission approval, submission state persistence, submitted state
  ...
  persistence, uncertainty resolution automation, paste target approval, paste
  target state, paste state persistence, paste state synchronization, paste
  success verification, active window detection, target-agent UI inspection,
  target content validation, clipboard target validation, paste automation,
  persisted destination state, persisted target state, handoff approval,
  handoff approval state, handoff state persistence, handoff state
  synchronization, merge decision writes, merge approval, merge state
  persistence, merge state synchronization, merge readiness recalculation,
  background collection, hidden recovery actions, submission event hooks,
  safety approval state, Codex or Claude Code UI automation, hidden prompt
  submission, command execution, filesystem reads/writes, persisted review
  state, checklist completion state, background analysis, external model calls,
  or new write tools
- prompt bodies, transcript content, compact summaries, outcome summaries,
  evidence refs, evidence bodies, raw paths, provider credentials, target
  content, active-window titles, pasted content, paste result state, submitted
...
### Slice 4.123: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Renewed-Memory-Approval Pre-Merge Freshness Advisory

Decision:

- Selected worktree detail should remind the operator to review post-submit
  retry renewed-memory-approval freshness uncertainty before merge decisions.
...
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry renewed-memory-approval
  collection uncertainty reminder and before paste destination guidance
- focused server/API/web tests proving it is present
...
- submission approval, submission state persistence, submitted state
  ...
  persistence, post-submit retry renewed-memory-approval collection result
  persistence, collection result verification, collection uncertainty result
  persistence, freshness verification, merge decision writes, merge approval,
  merge state persistence, merge state synchronization, merge readiness
  recalculation, paste target approval, paste target state, paste state
  persistence, paste state synchronization, paste success verification, active
  window detection, target-agent UI inspection, target content validation,
  clipboard target validation, paste automation, persisted destination state,
  persisted target state, handoff approval, handoff approval state, handoff
  state persistence, handoff state synchronization, background collection,
  hidden recovery actions, submission event hooks, safety approval state, Codex
  or Claude Code UI automation, hidden prompt submission, command execution,
  filesystem reads/writes, persisted review state, checklist completion state,
  background analysis, external model calls, or new write tools
- prompt bodies, transcript content, compact summaries, outcome summaries,
  evidence refs, evidence bodies, raw paths, provider credentials, target
...
### Slice 4.124: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Renewed-Memory-Approval Pre-Handoff Freshness Advisory
...
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry renewed-memory-approval
  pre-merge freshness advisory and before paste destination guidance
- focused server/API/web tests proving it is present
...
### Slice 4.126: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Renewed-Memory-Approval Pre-Submit Freshness Advisory
...
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry renewed-memory-approval
  pre-paste freshness advisory and before paste destination guidance
- focused server/API/web tests proving it is present
...
- prompt bodies, transcript content, compact summaries, outcome summaries,
  ...
  approval state, renewed memory approval state, approval result state, merge
  decision state, handoff approval state, paste target state, paste result
  state, submission approval state, or secret-looking tokens
- package/plugin/slash/hook/MCP rename work

### Slice 4.127: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Renewed-Memory-Approval Post-Submit Freshness Advisory
...
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry renewed-memory-approval
  pre-submit freshness advisory and before paste destination guidance
- focused server/API/web tests proving it is present

Do not add:

- submitted state persistence, submitted state synchronization, agent response
  monitoring, agent response persistence, freshness monitoring state,
  post-submit freshness result persistence, automatic collection, collection
  command execution, retry automation, retry command execution, git status
  reads, transcript inspection, agent UI monitoring, active window detection,
  target-agent UI inspection, submit automation, Enter key automation, button
  clicking, hidden prompt submission, safety approval state, submission
  approval, submission approval state, memory approval writes, memory approval
  state, renewed memory approval state, approval result persistence, approval
  state synchronization, merge decision writes, paste target state, paste result
  state, persisted review state, checklist completion state, external model
  calls, filesystem reads/writes, or new write tools
- prompt bodies, transcript content, compact summaries, outcome summaries,
  evidence refs, evidence bodies, raw paths, provider credentials, target
  content, active-window titles, pasted content, paste result state, submitted
  state, agent response content, collection result state, post-submit
  ...
  state, approval result state, memory approval state, renewed memory approval
  state, merge decision state, handoff approval state, submission approval
  state, or secret-looking tokens
- package/plugin/slash/hook/MCP rename work
...
### Slice 4.129: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Renewed-Memory-Approval Post-Submit Collection Uncertainty Reminder

Decision:

- Selected worktree detail should remind the operator to collect a new explicit
  loop snapshot when post-submit collection result after post-submit retry
...
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry renewed-memory-approval
  post-submit collection result non-persistence note and before paste
  destination guidance
- focused server/API/web tests proving it is present
...
- collection result persistence, collection result synchronization,
  collection result verification, post-submit collection result persistence,
  post-submit collection result verification, post-submit retry renewed-memory
  approval post-submit collection result persistence, post-submit retry
  renewed-memory approval post-submit collection result verification,
  submitted state persistence, submitted state synchronization, agent response
  monitoring, agent response persistence, freshness monitoring state,
  automatic collection, collection command execution, retry automation, retry
  command execution, git status reads, transcript inspection, agent UI
  monitoring, active window detection, target-agent UI inspection, submit
  automation, Enter key automation, button clicking, hidden prompt submission,
  safety approval state, submission approval, submission approval state, memory
  approval writes, memory approval state, renewed memory approval state,
  approval result persistence, approval state synchronization, merge decision
  writes, paste target state, paste result state, persisted review state,
  checklist completion state, external model calls, filesystem reads/writes, or
  new write tools
- prompt bodies, transcript content, compact summaries, outcome summaries,
  evidence refs, evidence bodies, raw paths, provider credentials, target
  content, active-window titles, pasted content, paste result state, submitted
  ...
  state, post-submit retry renewed-memory-approval collection result state,
  post-submit retry renewed-memory-approval post-submit collection result
  state, post-submit retry renewed-memory-approval post-submit collection
  uncertainty result state, uncertainty result state, freshness result state,
  retry freshness result state, approval result state, memory approval state,
  renewed memory approval state, merge decision state, handoff approval state,
  submission approval state, or secret-looking tokens
- package/plugin/slash/hook/MCP rename work

### Slice 4.130: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Renewed-Memory-Approval Post-Submit Collection Pre-Merge Freshness Advisory
...
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry renewed-memory-approval
  post-submit collection uncertainty reminder and before paste destination
  guidance
- focused server/API/web tests proving it is present

Do not add:

- merge approval, merge decision persistence, freshness verification, freshness
  result persistence, collection result persistence, collection result
  synchronization, collection result verification, post-submit collection
  result persistence, post-submit collection result verification, post-submit
  retry renewed-memory approval post-submit collection result persistence,
  submitted state persistence, submitted state synchronization, agent response
  monitoring, agent response persistence, freshness monitoring state,
  automatic collection, collection command execution, retry automation, retry
  command execution, git status reads, transcript inspection, agent UI
  ...
  safety approval state, submission approval, submission approval state, memory
  approval writes, memory approval state, renewed memory approval state,
  approval result persistence, approval state synchronization, paste target
  state, paste result state, persisted review state, checklist completion
  state, external model calls, filesystem reads/writes, or new write tools
- prompt bodies, transcript content, compact summaries, outcome summaries,
  evidence refs, evidence bodies, raw paths, provider credentials, target
  content, active-window titles, pasted content, paste result state, submitted
  state, agent response content, collection result state, post-submit
  ...
  post-submit retry renewed-memory-approval post-submit collection result
  state, post-submit retry renewed-memory-approval post-submit collection
  uncertainty result state, post-submit retry renewed-memory-approval
  post-submit collection freshness result state, uncertainty result state,
  freshness result state, retry freshness result state, approval result state,
  memory approval state, renewed memory approval state, merge decision state,
  handoff approval state, submission approval state, or secret-looking tokens
- package/plugin/slash/hook/MCP rename work

### Slice 4.131: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Renewed-Memory-Approval Post-Submit Collection Pre-Handoff Freshness Advisory
...
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry renewed-memory-approval
  post-submit collection pre-merge freshness advisory and before paste
  destination guidance
- focused server/API/web tests proving it is present

Do not add:

- handoff approval, handoff decision persistence, freshness verification,
  freshness result persistence, collection result persistence, collection
  result synchronization, collection result verification, post-submit
  collection result persistence, post-submit collection result verification,
  post-submit retry renewed-memory approval post-submit collection result
  persistence, submitted state persistence, submitted state synchronization,
  agent response monitoring, agent response persistence, freshness monitoring
  state, automatic collection, collection command execution, retry automation,
  retry command execution, git status reads, transcript inspection, agent UI
  monitoring, active window detection, target-agent UI inspection, submit
  automation, Enter key automation, button clicking, hidden prompt submission,
  safety approval state, submission approval, submission approval state, memory
  approval writes, memory approval state, renewed memory approval state,
...
### Slice 4.132: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Renewed-Memory-Approval Post-Submit Collection Pre-Paste Freshness Advisory
...
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry renewed-memory-approval
  post-submit collection pre-handoff freshness advisory and before paste
  destination guidance
- focused server/API/web tests proving it is present

Do not add:

- paste target approval, paste decision persistence, freshness verification,
  freshness result persistence, collection result persistence, collection
  result synchronization, collection result verification, post-submit
  collection result persistence, post-submit collection result verification,
  post-submit retry renewed-memory approval post-submit collection result
  ...
  state, automatic collection, collection command execution, retry automation,
  retry command execution, git status reads, transcript inspection, agent UI
  monitoring, active window detection, target-agent UI inspection, paste
  automation, submit automation, Enter key automation, button clicking, hidden
  prompt submission, safety approval state, submission approval, submission
  approval state, memory approval writes, memory approval state, renewed memory
  approval state, approval result persistence, approval state synchronization,
  paste target state, paste result state, persisted review state, checklist
  completion state, external model calls, filesystem reads/writes, or new write
  tools
- prompt bodies, transcript content, compact summaries, outcome summaries,
  evidence refs, evidence bodies, raw paths, provider credentials, target
  content, active-window titles, pasted content, paste result state, submitted
...
### Slice 4.133: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Renewed-Memory-Approval Post-Submit Collection Pre-Submit Freshness Advisory
...
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry renewed-memory-approval
  post-submit collection pre-paste freshness advisory and before paste
  destination guidance
- focused server/API/web tests proving it is present

Do not add:

- submission approval, submission decision persistence, freshness verification,
  freshness result persistence, collection result persistence, collection
  result synchronization, collection result verification, post-submit
  collection result persistence, post-submit collection result verification,
  post-submit retry renewed-memory approval post-submit collection result
  ...
  monitoring, active window detection, target-agent UI inspection, paste
  automation, submit automation, Enter key automation, button clicking, hidden
  prompt submission, safety approval state, memory approval writes, memory
  approval state, renewed memory approval state, approval result persistence,
  approval state synchronization, paste target state, paste result state,
  persisted review state, checklist completion state, external model calls,
  filesystem reads/writes, or new write tools
- prompt bodies, transcript content, compact summaries, outcome summaries,
  evidence refs, evidence bodies, raw paths, provider credentials, target
  content, active-window titles, pasted content, paste result state, submitted
  state, agent response content, collection result state, post-submit
...
### Slice 4.134: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Renewed-Memory-Approval Post-Submit Collection Post-Submit Freshness Advisory
...
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry renewed-memory-approval
  post-submit collection pre-submit freshness advisory and before paste
  destination guidance
- focused server/API/web tests proving it is present
...
- submitted state persistence, submitted state synchronization, agent response
  monitoring, agent response persistence, freshness monitoring state, freshness
  result persistence, collection result persistence, collection result
  synchronization, collection result verification, post-submit collection
  result persistence, post-submit collection result verification, post-submit
  retry renewed-memory approval post-submit collection result persistence,
  ...
  automation, submit automation, Enter key automation, button clicking, hidden
  prompt submission, safety approval state, submission approval, submission
  decision persistence, memory approval writes, memory approval state, renewed
  memory approval state, approval result persistence, approval state
  synchronization, paste target state, paste result state, persisted review
  state, checklist completion state, external model calls, filesystem
  reads/writes, or new write tools
- prompt bodies, transcript content, compact summaries, outcome summaries,
  evidence refs, evidence bodies, raw paths, provider credentials, target
  content, active-window titles, pasted content, paste result state, submitted
  state, agent response content, collection result state, post-submit
...
### Slice 4.135: Post-Memory-Approval Retry Renewed-Memory-Approval Post-Submit Retry Renewed-Memory-Approval Post-Submit Collection Freshness Result Non-Persistence Note
...
- web API typing and selected worktree detail rendering inside continuation
  guidance, immediately after post-submit retry renewed-memory-approval
  post-submit collection post-submit freshness advisory and before paste
  destination guidance
- focused server/API/web tests proving it is present

Do not add:

- freshness result persistence, freshness result synchronization, freshness
  result verification, freshness monitoring state, collection result
  persistence, collection result synchronization, collection result
  verification, post-submit collection result persistence, post-submit
  collection result verification, post-submit retry renewed-memory approval
  post-submit collection result persistence, submitted state persistence,
  submitted state synchronization, agent response monitoring, agent response
  persistence, automatic collection, collection command execution, retry
  automation, retry command execution, git status reads, transcript inspection,
  agent UI monitoring, active window detection, target-agent UI inspection,
  paste automation, submit automation, Enter key automation, button clicking,
  hidden prompt submission, safety approval state, submission approval,
  submission decision persistence, memory approval writes, memory approval
  state, renewed memory approval state, approval result persistence, approval
  state synchronization, paste target state, paste result state, persisted
  review state, checklist completion state, external model calls, filesystem
  reads/writes, or new write tools
- prompt bodies, transcript content, compact summaries, outcome summaries,
  evidence refs, evidence bodies, raw paths, provider credentials, target
  content, active-window titles, pasted content, paste result state, submitted

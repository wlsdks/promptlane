export const HOOK_COPY = {
  en: {
    ...
    blockedReason: (score: number, band: string, minScore: number) =>
      `promptlane blocked this prompt before submission because its local score was ${score}/100 (${band}), below the configured threshold ${minScore}.`,
    clipboardHit:
  ...
  ko: {
    ...
    blockedReason: (score: number, band: string, minScore: number) =>
      `promptlane가 이 프롬프트를 제출 전 차단했습니다. 로컬 점수 ${score}/100 (${band})가 설정된 기준 ${minScore} 미만입니다.`,
    clipboardHit:

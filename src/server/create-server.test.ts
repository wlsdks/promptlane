...
function createPersistedTestDataDir(): string {
  const dataDir = join(tmpdir(), `looprelay-server-${randomUUID()}`);
...
describe("createServer P2 ingest boundary", () => {
  ...
  it("issues local web sessions and requires csrf for cookie delete", async () => {
    ...
    expect(cookie).toContain("looprelay_session=");
  ...
  it("guides missing prompt usage-event users back to local archive search", async () => {
    const storage = createMemoryStorage();
    const server = createTestServer({ storage });
    const session = await server.inject({
      method: "GET",
    ...
    const response = await server.inject({
      method: "POST",
      url: "/api/v1/prompts/prmt_20260501_100000_deadbeefdead/events",
      headers: {
        host: "127.0.0.1:17373",
        cookie,
        "x-csrf-token": csrfToken,
      },
      payload: { type: "prompt_copied" },
    });
  ...
  it("guides web loop brief users to capture the first loop before retrying", async () => {
    ...
    expect(response.body).toContain(
      "No loop snapshot found. Send one Codex or Claude Code prompt, run `looprelay coach` to confirm the first score, then run `looprelay loop collect` before retrying `looprelay loop brief`.",
    );
    expect(response.body).not.toContain("/Users/example");
    expect(response.body).not.toContain("sk-proj-secret");
  });
  ...
  it("returns the selected worktree review packet summary read-only", async () => {
    ...
    expect(response.statusCode).toBe(200);
    expect(response.json().data.continuation_safety).toEqual({
    ...
    expect(response.json().data).not.toHaveProperty("paste_destination");
    expect(response.json().data).not.toHaveProperty("handoff_checklist");
    expect(response.json().data).not.toHaveProperty(
    ...
    expect(response.json()).toMatchObject({
      data: {
        ...
        selected_brief_action: {
          ...
          command: "looprelay loop brief --worktree review-worktree",
          writes_files: false,
          external_calls: false,
        },
        command_distinction: {
        ...
        review_packet_summary: {
          ...
          command_hint: {
            ...
            command:
              "looprelay loop brief --worktree review-worktree --branch codex/agent-loop-memory-design",
            provenance: {
  ...
  it("guides web memory approval users to capture evidence before approving memory", async () => {
    ...
    expect(response.statusCode).toBe(404);
    expect(response.body).toContain(
      "No loop snapshot found. Send one Codex or Claude Code prompt, run `looprelay coach` to confirm the first score, run `looprelay loop collect`, then record a passed loop outcome with safe evidence before retrying `looprelay loop memory-approve`.",
    );
    expect(response.body).not.toContain("/Users/example");
    expect(response.body).not.toContain("sk-proj-secret");
  });
  ...
  it("guides web instruction patch users to approve evidence-backed memory first", async () => {
    ...
    expect(response.statusCode).toBe(404);
    expect(response.body).toContain(
      "No loop memory found. Capture one Codex or Claude Code prompt, confirm the first score, collect a loop snapshot, record a passed outcome with safe evidence, then run `looprelay loop memory-approve` before retrying `looprelay loop instruction-patch --target-file AGENTS.md`.",
    );
    expect(response.body).not.toContain("/Users/example");
    expect(response.body).not.toContain("sk-proj-secret");
  });
  ...
  it("normalizes, redacts, and stores a valid Claude Code fixture", async () => {
    ...
    expect(storage.events[0]?.event.cwd).toBe(
      "/Users/example/side-project/looprelay",
    );
  ...
  it("normalizes, redacts, and stores a valid Codex fixture", async () => {
    ...
    expect(storage.events[0]?.event).toMatchObject({
      ...
      turn_id: "turn-456",
      cwd: "/Users/example/side-project/looprelay",
      transcript_path: "/Users/example/.codex/sessions/looprelay.jsonl",
      model: "gpt-5.5",
  ...
  it("guides missing improvement draft copy users back to saved drafts", async () => {
    const storage = createMemoryStorage();
    storage.promptDetails.push(
      promptDetail({ id: "prmt_20260501_100000_abcdefabcdef" }),
    );
    ...
    const response = await server.inject({
      method: "POST",
      url: "/api/v1/prompts/prmt_20260501_100000_abcdefabcdef/improvements/impdraft_missing/copy",
      headers: {
        host: "127.0.0.1:17373",
        cookie,
        "x-csrf-token": csrfToken,
    ...
    expect(detail).not.toContain("impdraft_missing");
    expect(response.body).not.toContain("/Users/example");
    expect(response.body).not.toContain("sk-proj-secret");
  });
...
function createTestServer(options: TestServerOptions = {}) {
  return createServer({
    dataDir: options.dataDir ?? "/tmp/looprelay-test",

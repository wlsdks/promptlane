...
export type AskClarifyingQuestionsToolArguments = {
   ...
   * LOOPRELAY_NATIVE_DIALOG=1 environment variable opts in implicitly.
   */
...
const ASK_CLARIFYING_REQUEST_MESSAGE =
  "Please answer these clarifying questions before looprelay composes the rewrite.";
...
export const ASK_CLARIFYING_QUESTIONS_TOOL_DEFINITION: LoopRelayMcpToolDefinition =
  {
    name: "ask_clarifying_questions",
    description:
      "Score the supplied prompt with looprelay's local rules; if any prompt-quality axis is missing, ask the user a small set of clarifying questions through MCP elicitation/create and compose the final approval-ready rewrite from the user's verbatim answers. Falls back to returning clarifying_questions metadata when the client does not advertise elicitation support, when the user declines, or when the elicitation request times out — never auto-submits a rewrite. Local-only, copy-based, no external LLM calls, no input storage.",
    annotations: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        allow_native_dialog: {
          type: "boolean",
          description:
            "Opt in to a local OS-native dialog fallback when the MCP client does not advertise elicitation support. Defaults to false unless LOOPRELAY_NATIVE_DIALOG=1 is set.",
        },
      },
...
function shouldUseNativeFallback(
  ...
  return process.env.LOOPRELAY_NATIVE_DIALOG === "1";
}
